...
def load_client_results() -> dict:
    ...
    for path in sorted(glob.glob(pattern)):
        ...
        # Handle dua format JSON:
        # Format 1 (demo): [{"round":1, "accuracy":0.9, ...}, ...]
        # Format 2 (FL sungguhan): {"client_id": 1, "rounds": [...]}
        if isinstance(data, list):
            rounds = data
        elif isinstance(data, dict) and "rounds" in data:
            rounds = data["rounds"]
            # FL sungguhan menyimpan local_metrics dan global_metrics
            # Ekstrak global_metrics jika ada
            normalized = []
            for r in rounds:
                if "global_metrics" in r:
                    normalized.append({
                        "round":    r["round"],
                        "accuracy": r["global_metrics"].get("test_accuracy", 0),
                        "f1_macro": r["global_metrics"].get("test_f1", 0),
                    })
                elif "local_metrics" in r:
                    normalized.append({
                        "round":    r["round"],
                        "accuracy": r["local_metrics"].get("test_accuracy", 0),
                        "f1_macro": r["local_metrics"].get("test_f1", 0),
                    })
                else:
                    normalized.append(r)
            rounds = normalized
        ...
        if rounds:
            results[f"Client {client_id}"] = rounds
...
def plot_accuracy_per_round(client_results: dict):
    ...
    plt.title("Akurasi per Ronde — Federated Learning\n(Dataset: Student Performance)")
...
def plot_f1_per_round(client_results: dict):
    ...
    plt.title("F1-Score per Ronde — Federated Learning")
    plt.xlabel("Ronde Federasi")
